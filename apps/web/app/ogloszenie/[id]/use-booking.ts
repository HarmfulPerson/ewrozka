...
export function useBooking(advertisementId: number | undefined) {
  ...
  const loadSlotDates = async () => {
    ...
    try {
      ...
      const response = await apiGetAvailableSlots(advertisementId, fromDate, toDate);
  ...
  const loadSlotsForDate = async (dateStr: string) => {
    ...
    try {
      ...
      const response = await apiGetAvailableSlots(advertisementId, dateStr, toDate);
